...
uint32_t tcp::tcp_data_len(u_int32_t tcp_hdr_len, uint32_t ip_total_len, uint32_t ip_hdr_len, u_int8_t flags) {
    using namespace protocol::tcp;
...
std::string_view tcp::tcp_state_to_string(State state) {
    switch (state) {
        case State::UNKNOWN: {
        ...
        case State::SYN_SENT: {
            static std::string syn_sent = "SYN_SENT";
            return syn_sent;
        }
        case State::SYN_RECIEVED: {
            static std::string syn_received = "SYN_RECEIVED";
            return syn_received;
        }
        case State::ESTABLISHED: {
            static std::string established = "ESTABLISHED";
            return established;
        }
        case State::FIN_WAIT1: {
            static std::string fin_wait1 = "FIN_WAIT1";
            return fin_wait1;
        }
        case State::FIN_WAIT2: {
            static std::string fin_wait2 = "FIN_WAIT2";
            return fin_wait2;
        }
        case State::CLOSE_WAIT: {
            static std::string close_wait = "CLOSE_WAIT";
            return close_wait;
        }
        case State::CLOSING: {
            static std::string closing = "CLOSING";
            return closing;
        }
        case State::LAST_ACK: {
            static std::string last_ack = "LAST_ACK";
            return last_ack;
        }
        case State::TIME_WAIT: {
            static std::string time_wait = "TIME_WAIT";
            return time_wait;
        }
        case State::CLOSED: {
        ...
        default: {
            static std::string invalid = "INVALID_STATE";
            return invalid;
        }
    }
}
